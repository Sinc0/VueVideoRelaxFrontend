        ...
        <div id="activeRooms" v-if="vuexActiveRooms.length > 0">
            ...
            <div class="room" v-for="room in vuexActiveRooms" v-bind:key="room.key">
                <router-link v-bind:to="'/' + room.room" v-if="currentRoute.substr(1) == room.room && room.room != 'temp'">
                    <div id="currentRoom" class="roomLink">({{room.clients.length}}) {{room.room}}</div>
                </router-link>
...
<style scoped>
 ...
 #activeRooms { display: inline-flex; flex-direction: column; overflow: hidden; scrollbar-width: thin; border: 0px solid white; } 
 #defaultRooms 
 { 
    display: inline-flex; 
 ...
 #activeRoomsTitle { width: auto; margin: 4px 0px 0px 0px; padding: 0px; opacity: 0.8; font-size: 20px; }
 #defaultRoomsTitle { width: auto; margin: 20px 0px 0px 0px; padding: 0px; opacity: 0.8; font-size: 20px; } 
 ...
 {
    #componentNavbarRoomsMobile { display: block; height: 100vh; width: 100vw; left: 0px; background-color: #1c1b1b;  z-index: 3; padding-bottom: 20px; }
